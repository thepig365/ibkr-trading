...
- [ ] 2. 浏览器 `http://127.0.0.1:8765/dashboard` — **交易员驾驶舱**（**交易日摘要** + **默认折叠「开发者与引擎诊断」中的数据质量与控制区**）；**只有已记录平仓的记录**才算入绩效曲线与详细美元/R 统计。**缺复盘图通常为本地 1m K 不齐** — 请到 **`/reports`/`/trades`** 或通过摘要区「**补齐交易图表**」表单（要点按钮；**不会因打开 Dashboard 而自动 fetch**）。需要路径/`stdout`/引擎门控时再展开下方「开发者与引擎诊断」（含 Today's safety / auto loop readiness / Automatic engine）。若要用 **Automatic Paper Trading Engine**：在折叠区内看 **Check Automatic Engine Readiness**（`automatic-paper-engine-readiness --json`）等。旧版 **60 分钟烟测准备**仍为 **Automatic paper loop readiness**。
