...
- [ ] 2. 浏览器 `http://127.0.0.1:8765/dashboard` — **交易员控制台**（今日已平仓 R、累计 R、账本与快捷入口）；需要路径/stdout 时再展开「开发者与引擎诊断」。健康/预算/当前**纸面策略**提示仍在页内；若使用 **Automatic Paper Trading Engine**：先看 **Check Automatic Engine Readiness**（`automatic-paper-engine-readiness --json`）或 `run-automatic-paper-engine --dry-run --json`。旧版 **60 分钟烟测准备**读数仍用 **Automatic paper loop readiness** / `auto-loop-readiness`（含 `READY_FOR_PAPER_TEST`，与引擎门控可能不一致）。  
