...
- [ ] 2. 浏览器 `http://127.0.0.1:8765/dashboard` — **先看首屏「交易日记核心」**：**累计 R 曲线**（策略健康主指标）、**美元资金曲线**（仅可靠 USD 时出现）、**Edge 健康度**、**数据质量**、**跳过原因**；再看 **交易日摘要** 与 **需要处理**（未刷新券商快照时会提示连接 TWS）。**只有已记录平仓**才进入 R 曲线与 Edge 统计；美元权益曲线**不会**在缺字段时伪造。**缺复盘图**通常为本地 1m 不齐 — 到 **`/reports`/`/trades`** 或「**补齐交易图表**」白名单（**不因打开 Dashboard 自动 fetch**）。路径/`stdout`/`launchd`/自动引擎门控仍在 **折叠「开发者与引擎诊断」**（含 Today's safety / readiness / Automatic engine）。若要用 **Automatic Paper Trading Engine**：在折叠区内 **Check Automatic Engine Readiness**（`automatic-paper-engine-readiness --json`）等。旧版 **60 分钟烟测**仍为 **Automatic paper loop readiness**。  
...
- [ ] 2a-bg.（Mac，可选）**后台 launchd**：`bash scripts/install_full_auto_paper_launchd.sh` 后无需一直开 **Cursor** 或 **Terminal**；TWS 纸面 + Mac 勿睡。查状态：`bash scripts/status_full_auto_paper_launchd.sh`；停：`bash scripts/uninstall_full_auto_paper_launchd.sh` 或 UI **Kill switch**。plist 源文件：`scripts/com.strategy-lab.full-auto-paper.plist`（勿提交到 LaunchAgents 的已填路径版本）。  
